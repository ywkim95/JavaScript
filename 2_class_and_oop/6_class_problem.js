...
console.log(new Country('대한민국',iveMembers));
console.log(new IdolGroup('BTS',btsMembers));
let ive = iveMembers.map((i) => new Idol(i.name, i.year));
console.log(ive);
let bts = btsMembers.map((i) => new MaleIdol(i.name,i.year));
console.log(bts);
for(let i=0;i<bts.length;i++) {
    console.log(bts[i].sing());
}
let ive2 = iveMembers.map((i)=>new FemaleIdol(i.name,i.year));
for(let i = 0; i<ive2.length;i++){
    console.log(ive2[i].dance());
}